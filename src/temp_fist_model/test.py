...
def find_sub_frame(draw_matrix):
    ...
    for cur_size in range(MATRIX_SIZE // 2, MATRIX_SIZE + 1, MATRIX_SIZE // 6):
        for i in range(0, MATRIX_SIZE - cur_size + 1, MATRIX_SIZE // 10):
            for j in range(0, MATRIX_SIZE - cur_size + 1, MATRIX_SIZE // 10):
                sub_frame = draw_matrix[i : i+cur_size, j : j + cur_size]
                cur_bits = np.sum(sub_frame)
                if (total_bits - cur_bits < 5):
...
while True:
    ...
    if (time.time() - last_detect_time > TIMEOUT and detected):
        print("No detection for 3 seconds. Exiting...")
